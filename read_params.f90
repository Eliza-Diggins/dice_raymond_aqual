...
subroutine read_params
  ...
  write(*,*)' '
  write(*,*)'        RAyMOND AQUAL patch, written by Graeme Candlish        '
...
106 continue
  ...
  !RAyMOND
  if(maxstoredcells==0)then
     if(myid==1)write(*,*)'Error in the namelist:'
     if(myid==1)write(*,*)'Allocate memory for storage of interpolated points (maxstoredcells in MOND_PARAMS).'
     nml_ok=.false.
  endif
